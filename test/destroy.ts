...
t.equal(
  new MP().destroy().destroyed,
  true,
  'destroy() sets .destroyed getter'
)
...
t.test('calls close if present', t => {
  const mp = new MP() as MP & { close(): void }
...
t.test('destroy with no error rejects a promise', t => {
  const mp = new MP()
  const ret = t.rejects(() => mp.promise(), {
...
t.test('pipe after destroy is a no-op', t => {
  ...
  p.destroy()
  const q = new MP()
  q.on('data', () => t.fail('should not get data, upstream is destroyed'))
  p.pipe(q)
  t.end()
})
...
t.test('read after destroy always returns null', t => {
  const p = new MP<string>({ encoding: 'utf8' })
  p.write('hello, ')
  p.write('world')
